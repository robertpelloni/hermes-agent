...
func initSQLite() *sqliteDB {
	...
	path := filepath.Join(home, ".hermes", "sessions.db")
...
func (s *sqliteDB) saveMessage(sessionID, role, content string) error {
	_, err := s.db.Exec(`INSERT INTO conversations (session_id, role, content, created_at) VALUES (?, ?, ?, ?)`,
		sessionID, role, content, time.Now().UTC())
	return err
}

func (s *sqliteDB) loadMessages(sessionID string) ([]MessageRecord, error) {
	rows, err := s.db.Query(`SELECT id, role, content, created_at FROM conversations WHERE session_id = ? ORDER BY id ASC`, sessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
