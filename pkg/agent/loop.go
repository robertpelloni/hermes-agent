...
func (a *Agent) RunConversationLoop(ctx context.Context, platform, userID, text string, memStore *memory.Store) (string, error) {
	...
	response := fmt.Sprintf("I received your message on %s: %s", platform, text)
	...
	if memStore != nil {
		memStore.StoreUsageAttribution(fmt.Sprintf("%s-%s", platform, userID), "hermes-default-model", tokensUsed, durationMs)
	}
